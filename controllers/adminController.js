...
exports.getSellerCashouts = async (req, res) => {
  try {
    ...
    const totalRecords = await SellerCashout.countDocuments(query);
    const pageCount = Math.ceil(totalRecords / limitNumber);
...
exports.getSellerCashoutDetails = catchAsync(async (req, res, next) => {
  ...
  const cashout = await SellerCashout.findById(cashoutId).populate({
    path: "sellerWalletId",
    populate: {
      path: "sellerId",
      model: "Seller",
