...
async function CartToOrder(
  ...
  try {
    ...
    // }).populate("cart"); // Populate the 'cart' field
    ...
    //   const sku = await Sku.findById(productItem.skuId).populate("productId");
    //   if (sku) {
    //     let item = { ...sku.productId };
    ...
    //   { $unwind: "addresses"},
    //   { $match: { userId: user._id, "addresses.defaultAddress": true } },
    //   { $limit: 1 }
    // ])
...
exports.paymentVerification = async (req, res) => {
  ...
  if (isAuthentic) {
    ...
    res.redirect(
      `https://chaperoneplants.com/paymentsuccess?reference=${razorpay_payment_id}`
    );
  } else {
    res.status(400).json({
      success: false,
...
exports.paymentCheckout = async (req, res, next) => {
  ...
  try {
    ...
    var options = {
      amount: Number(req.body.totalValue * 100), // amount in the smallest currency unit
      currency: "INR",
