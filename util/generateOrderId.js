...
const generatePartnerId = async () => {
  ...
  const numericWithDashes = numericPart.slice(0, 2) + '-' + numericPart.slice(2);

  // Generate two random uppercase letters for uniqueness
  const alphabets = String.fromCharCode(Math.floor(Math.random() * 26) + 65) +
...
module.exports={
    generateOrderId,generateBookingId,generatePartnerId
}
